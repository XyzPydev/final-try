...
async def show_quests(client: Client, message: Message):
    ...
    if all_quests_completed(user_id):
        await message.reply(
            f"🔥 Поздравляем! Вы завершили все квесты!\n\nСсылка на 2 часть ивента: {encrypt_user_id(user_id)}",
            quote=True,
        )
        return
...
craft_recipes = {
    ...
    "stone_plate": {
        "items": {"stone": 5, "stone_sediments": 10},
        "result": 2,
    },  # каменная плита
...
drill_motor_levels_price = [
    ...
    {"level": 2, "price": 2000, "items": {"stone_sediments": 10, "iron_ingots": 5}},
    {
        "level": 3,
        "price": 5000,
        "items": {"stone_sediments": 15, "iron_ingots": 10, "gold_bars": 2},
    },
]
...
async def wipe_callback(client, callback_query):
    ...
    await callback_query.message.edit_text(
        "ТОЧНО??", reply_markup=InlineKeyboardMarkup(rows)
    )
...
async def set_lose_monet(user_id, amount):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE crash SET lose_count = lose_count + ? WHERE id = ?",
...
async def set_win_monet(user_id, amount):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE crash SET win_count = win_count + ? WHERE id = ?",
        (float(amount), user_id),
    )
    db.commit()
    db.close()
...
async def craft_item(user_id, item_name):
    ...
    for resource, amount in recipe["items"].items():
        cursor.execute(
            f"UPDATE crash SET {resource} = {resource} - ? WHERE id = ?",
            (amount, user_id),
        )

    # Добавляем результат
    cursor.execute(
        f"UPDATE inv SET {item_name} = {item_name} + ? WHERE id = ?",
        (recipe["result"], user_id),
    )
...
async def upgrade_drill_component(user_id, component, levels_price):
    ...
    next_level_data = next(
        (lvl for lvl in levels_price if lvl["level"] == current_level + 1), None
    )
    ...
    cursor = db.cursor()
    cursor.execute(
        f"UPDATE drill SET {component} = ? WHERE user_id = ?",
        (current_level + 1, user_id),
    )
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?",
        (float(next_level_data["price"]), user_id),
    )
    for item, amount in next_level_data["items"].items():
        cursor.execute(
            f"UPDATE inv SET {item} = {item} - ? WHERE id = ?", (amount, user_id)
        )
    db.commit()
    db.close()
...
async def refill_oil(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE drill SET oil = ? WHERE user_id = ?",
        (drill_data["max_oil_engine"], user_id),
    )
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?", (float(cost), user_id)
    )
    db.commit()
    db.close()
...
async def refill_cooling(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE drill SET cooling = ? WHERE user_id = ?",
        (drill_data["max_cooling_engine"], user_id),
    )
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?", (float(cost), user_id)
    )
...
async def repair_drill(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE drill SET heal_drill_engine = 100 WHERE user_id = ?", (user_id,)
    )
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?", (float(cost), user_id)
    )
        ...
        [
            [
                InlineKeyboardButton(
                    "Пополнить", callback_data=f"on_refill_energy_{user_id}"
                )
            ]
        ]
    )
...
async def on_refill_energy(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE drill SET energy = ? WHERE user_id = ?",
        (drill_data["max_energy"], user_id),
    )
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?", (float(cost), user_id)
    )
...
async def drill_command(client, message):
    ...
    if not drill_data:
        await message.reply(
            "У вас нет дрели. Купите ее с помощью /buy_drill за 5000 монет!"
        )
        return
    ...
    text += (
        f"❄️ Охлаждение: {drill_data['cooling']}/{drill_data['max_cooling_engine']}\n"
    )
    ...
    markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "Улучшить", callback_data=f"drill_upgrade_{user_id}"
                ),
                InlineKeyboardButton(
                    "Добавить запчасть", callback_data=f"drill_parts_{user_id}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "Управление", callback_data=f"drill_manage_{user_id}"
                )
            ],
        ]
    )
...
async def craft_menu(client, callback_query):
    ...
    if callback_query.message.chat.id != user_id:
        ...
        await callback_query.message.edit_text(
            "Крафт доступен только в ЛС бота.", reply_markup=markup
        )
        return
    ...
    for item_name, recipe in craft_recipes.items():
        if can_craft(user_id, item_name):
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        f"Скрафтить {item_name}", callback_data=callback_data
                    )
                ]
            )
        else:
    ...
    if not buttons:
        text += (
            "Нет доступных предметов для крафта.\nДобывайте ресурсы с помощью /mine!"
        )
...
async def craft_item_callback(client, callback_query):
    ...
    if len(parts) < 4 or parts[1] != "item" or not parts[-1].isdigit():
        await callback_query.message.edit_text(
            f"Ошибка: неверный запрос крафта ({callback_query.data})."
        )
        return
    ...
    if item_name not in craft_recipes:
        await callback_query.message.edit_text(
            f"Ошибка: неизвестный предмет ({item_name})."
        )
        return
    ...
    if can_craft(user_id, item_name):
        ...
        markup = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Назад", callback_data=f"craft_menu_{user_id}")]]
        )
        await callback_query.message.edit_text(
            f"🎉 Вы скрафтили {item_name}!", reply_markup=markup
        )
    else:
        recipe = craft_recipes[item_name]
        required = ", ".join(f"{k}: {v}" for k, v in recipe["items"].items())
        await callback_query.message.edit_text(
            f"Недостаточно ресурсов для {item_name}. Требуется: {required}"
        )
...
async def drill_upgrade_menu(client, callback_query):
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                "Питание", callback_data=f"upgrade_power_source_{user_id}"
            )
        ],
...
async def drill_manage_menu(client, callback_query):
    ...
    buttons = [
        [
            InlineKeyboardButton(
                "Пополнить масло", callback_data=f"manage_oil_{user_id}"
            )
        ],
        [InlineKeyboardButton("Охладить", callback_data=f"manage_cooling_{user_id}")],
        [
            InlineKeyboardButton(
                "Пополнит Энергию", callback_data=f"manage_energy_{user_id}"
            )
        ],
        [
            InlineKeyboardButton(
                "Починить бур", callback_data=f"manage_repair_{user_id}"
            )
        ],
    ]
    markup = InlineKeyboardMarkup(buttons)
...
async def mine_command(client, message):
    ...
    available_resources = next(
        tier
        for lvl, tier in sorted(resource_tiers.items(), reverse=True)
        if calc_drill_lvl >= lvl
    )
...
def register_user(user_id, username, first_name):
    ...
    try:
        cursor.execute(
            "INSERT INTO crash (id, username, money, first_name) VALUES (?, ?, ?, ?)",
...
def get_user_farm(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT device, count, last_claim FROM farm WHERE user_id = ?", (user_id,)
    )
...
def claim_income(user_id):
    ...
    for device, data in farm.items():
        ...
        if count > 0:
            ...
            total_income += income
            cursor.execute(
                "UPDATE farm SET last_claim = ? WHERE user_id = ? AND device = ?",
                (now.isoformat(), user_id, device),
            )
    if total_income > 0:
        cursor.execute(
            "UPDATE crash SET money = money + ? WHERE id = ?",
            (float(total_income), user_id),
        )
    db.commit()
    db.close()
    return total_income
...
async def get_user_inventory(user_id):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT item_name, quantity FROM inv_user WHERE user_id = ?", (user_id,)
    )
...
def parse_bet_amount(
    arg: str, user_money: Optional[Union[int, float, str, Decimal]] = None
) -> int:
    if arg is None:
        return -1
...
async def requires_registration(func):
    async def wrapper(client, message):
        print(
            f"[DEBUG] Декоратор requires_registration для {func.__name__}, команда: {message.text}"
        )
...
async def set_user_drill_data(user_id, type, value):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE drill SET " + type + " =? WHERE user_id = ?", (value, user_id)
    )
    db.commit()
    db.close()
...
async def update_user_data(user_id, username, first_name):
    try:
        with sqlite3.connect(DBB) as db:  # Используем DBB вместо "users.db"
            ...
            if exists:
                cursor.execute(
                    "UPDATE crash SET username = ?, first_name = ? WHERE id = ?",
...
async def start_command(client, message):
    if message.chat.type == "private":
        ...
        await message.reply_text(
            "Привет! Чтобы узнать список команд, используйте /hb или список."
        )
...
async def admin_commands(client, message: Message):
    if (
        message.from_user.id not in API_OWNER
...
async def profile_command(client, message):
    ...
    if not user_data:
        await message.reply_text(
            "<b>Ошибка получения данных профиля, пропишите /ss</b>"
        )
        return
...
async def create_promo_command(client, message):
    ...
    if len(result) < 4:
        await message.reply_text(
            "<i>Используйте: /new_promo (название промо) (количество монет) (количество активаций)</i>"
        )
        return
    ...
    if existing:
        if existing[0] == 0:
            await message.reply_text(
                "<i>Этот промокод уже использовался и больше не активен. Вы не можете создать новый с таким же именем.</i>"
            )
        else:
            await message.reply_text(
    ...
    try:
        ...
        if message.from_user.id in API_OWNER:
            ...
            cursor.execute(
                "INSERT INTO promos (name, money, activations) VALUES (?, ?, ?)",
            ...
            await message.reply_text(
                "<i>Вы создаете промокод от имени обычного игрока...</i>"
            )
            value = money * activations
            user_data = get_user_data(message.from_user.id)
            if user_data["money"] < value:
                await message.reply_text(
                    f"<i>У вас недостаточно монет. Нужно еще {value - user_data['money']} монет.</i>"
                )
                return
            ...
            cursor.execute(
                "UPDATE crash SET money = money - ? WHERE id = ?",
                (float(value), message.from_user.id),
            )
            db.commit()
            bot_data = await client.get_me()
            await message.reply_text(
...
async def validate_promo_args(promo_name, money, activations):
    ...
    if not re.fullmatch(r"[a-zA-Z0-9]+", promo_name):
        return (
            None,
            "<i>Название промокода может содержать только латинские буквы и цифры.</i>",
        )
...
async def set_status_command(client, message):
    ...
    try:
        ...
        if status_id < 0 or status_id >= len(emojis):
            await message.reply_text(
                f"<i>Неверный идентификатор статуса. Допустимый диапазон: 0–{len(emojis) - 1}</i>"
            )
            return
    ...
    try:
        ...
        try:
            ...
            if member.status not in [
                ...
                await message.reply_text(
                    "<i>Вы должны быть участником чата, чтобы активировать промокод.</i>",
                    reply_markup=btn,
                )
                return
        except Exception:
        ...
        cursor.execute(
            "SELECT money, activations, is_active FROM promos WHERE name = ?",
            (promo_name,),
        )
        ...
        cursor.execute(
            "SELECT 1 FROM user_promos WHERE user_id = ? AND promo_name = ?",
            (user_id, promo_name),
        )
        if cursor.fetchone():
        ...
        if activations <= 0:
            cursor.execute(
                "UPDATE promos SET is_active = 0 WHERE name = ?", (promo_name,)
            )
            db.commit()
        ...
        cursor.execute(
            "UPDATE crash SET money = money + ? WHERE id = ?", (float(money), user_id)
        )
        cursor.execute(
            "INSERT INTO user_promos (user_id, promo_name) VALUES (?, ?)",
            (user_id, promo_name),
        )
        cursor.execute(
            "UPDATE promos SET activations = activations - 1 WHERE name = ?",
            (promo_name,),
        )
        db.commit()
        ...
        if remaining <= 0:
            ...
            db.commit()
            await message.reply_text(
                "<i>У этого промокода закончились активации и он был деактивирован.</i>"
            )
...
async def check_promo_command(client, message):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT money, activations, is_active FROM promos WHERE name = ?", (promo_name,)
    )
    promo = cursor.fetchone()
    db.close()
...
async def roulette_command(client, message: Message):
    try:
        ...
        if len(parts) < 3:
            await message.reply_text(
                "<i>Используйте: рул (сумма ставки) (ставка)</i>\nПример: рул 1k чет"
            )
            return
        ...
        if re.match(r"^\d+$", prediction_raw):
            ...
            if pred_num < 0 or pred_num > 36:
                await message.reply_text(
                    "<i>Некорректный номер. Допустимые значения: 0-36.</i>"
                )
                return
        else:
        ...
        winning_color = get_color(
            winning_number
        )  # ожидается 'красный'/'черный'/'зеленый'
        ...
        if pred_num is not None:
            ...
            if pred_str in ("кра", "красное", "red") and winning_color == "красный":
                ...
                pred_str in ("чет", "четное", "even", "чёт")
                and winning_even
                and winning_number != 0
            ):
                payout = bet_amount * 1.9
            elif (
                pred_str in ("нечет", "нечетное", "odd", "нечёт")
                and (not winning_even)
                and winning_number != 0
            ):
                payout = bet_amount * 1.9
                ...
                pred_str in ("зеро", "zero", "зеленый", "зеленое")
                and winning_number == 0
            ):
        ...
        if payout > 0:
            ...
            cursor.execute(
                "UPDATE crash SET money = money + ? WHERE id = ?", (net, user_id)
            )
            db.commit()
...
async def create_channel_promo_command(client, message):
    ...
    if len(args) != 4:
        await message.reply_text(
            "Используйте: /new_channel_promo (название) (сумма) (активации)"
        )
        return
    ...
    try:
        ...
        db.commit()
        await message.reply_text(
            f"✅ Промокод '{promo_name}' создан с наградой {money} монет и {activations} активациями."
        )
        ...
        channel_markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "Активировать",
                        url=f"http://t.me/Bfjsjdjndndbot?start=promo_{promo_name}",
                    )
                ]
            ]
        )
        ...
        await app.send_message(
            "GG_dangerizardhe", channel_text, reply_markup=channel_markup
        )
...
@app.on_message(
    filters.command("wheel") | filters.regex(r"колесо", flags=re.IGNORECASE)
)
async def wheel_command(client, message: Message):
    ...
    if user_id in active_wheel_players:
        await message.reply_text(
            "<i>Вы уже крутите колесо! Дождитесь окончания игры.</i>"
        )
        return
...
async def run_wheel_game(client, message: Message, bet_amount: int, user_id: int):
    ...
    try:
        ...
        if bet_amount < 1000000000:
            final_sector = rnd.choice(
                sectors_common if rnd.random() < (2 / 3) else sectors_rare
            )
        else:
            final_sector = rnd.choice(
                sectors_common if rnd.random() < (3 / 4) else sectors_rare
            )
...
@app.on_message(
    filters.command("lottery") | filters.regex(r"лотерея", flags=re.IGNORECASE)
)
async def lottery_command(client, message):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?", (float(bet), user_id)
    )
    db.commit()
    db.close()
    ...
    if max_count >= 3:
        ...
        cursor = db.cursor()
        cursor.execute(
            "UPDATE crash SET money = money + ? WHERE id = ?", (float(prize), user_id)
        )
        db.commit()
        db.close()
        await message.reply(
...
async def buy_status_command(client, message):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE crash SET money = money - ?, status = ? WHERE id = ?",
        (float(price), status_id, user_id),
    )
    db.commit()
    db.close()
    await message.reply_text(
        f"<b>Вы успешно купили статус {emojis[status_id]} за</b> <code>{price}</code> <b>монет!</b>"
    )
...
async def top_balance_callback(client, callback):
    ...
    keyboard = InlineKeyboardMarkup(
        [[btn] for key, btn in all_buttons.items() if key != category]
    )
...
async def top_balance_command(client, message):
    ...
    text = await format_top_message(
        top_users, "<b>🏆 Топ 10 игроков по балансу:</b>", order_by="money"
    )
    ...
    with sqlite3.connect(DBB) as db:
        ...
        if hidden == 1:
            ...
            return await message.reply(
                "🔓 Твой профиль сейчас <b>открыт</b> в топе.", reply_markup=keyboard
            )
...
@app.on_message(
    filters.regex(r"статус", flags=re.IGNORECASE)
    or filters.regex(r"статусы", flags=re.IGNORECASE)
)
...
"""
    ...
    await message.reply_text(
        main_text, reply_markup=kb, parse_mode=enums.ParseMode.HTML
    )
...
"""
        ...
        await callback_query.message.edit_text(
            text, reply_markup=nav, parse_mode=enums.ParseMode.HTML
        )
        ...
        for inv in data["inv"]:
            ...
            """,
                (
                    inv.get("id"),
                    inv.get("details", 0),
                    inv.get("stone_sediments", 0),
                    inv.get("iron_ingots", 0),
                    inv.get("gold_bars", 0),
                    inv.get("diamond_cores", 0),
                    inv.get("emerald_dust", 0),
                    inv.get("iron_ingots_dense", 0),
                    inv.get("gold_nuggets", 0),
                ),
            )
    ...
    if "drill" in data:
        for drill in data["drill"]:
            ...
            """,
                (
                    drill.get("user_id"),
                    drill.get("motor_lvl", 1),
                    drill.get("drill_head_lvl", 1),
                    drill.get("frame", 1),
                    drill.get("power_source", 1),
                    drill.get("handle", 0),
                    drill.get("cooling", 0),
                    drill.get("gearbox", 0),
                    drill.get("oil", 0),
                    drill.get("energy", 10),
                    drill.get("max_energy", 10),
                    drill.get("max_oil_engine", 10),
                    drill.get("max_cooling_engine", 10),
                    drill.get("heal_drill_engine", 100),
                ),
            )
...
async def transfer_command(client, message):
    try:
        ...
        if len(parts) != 3 and not message.reply_to_message:
            await message.reply_text(
                "<i>Используйте: /перевести (количество монет) (ID/@username пользователя)</i>\nИли ответьте на сообщение пользователя."
            )
            return
        ...
        if len(parts) == 3:
            ...
            if recipient_id.startswith("@"):
                ...
                cursor = db.cursor()
                cursor.execute(
                    "SELECT id FROM crash WHERE username = ?", (recipient_username,)
                )
                ...
                if recipient_data:
                    ...
                    await message.reply_text(
                        "<i>Получатель с таким юзернеймом не найден.</i>"
                    )
                    return
            else:
                try:
        ...
        if recipient_id == user_id:
            await message.reply_text(
                "<i>Вы не можете перевести монеты самому себе.</i>"
            )
            return
        ...
        if user_data["money"] < amount:
            await message.reply_text(
                "<i>Недостаточно монет на балансе для перевода.</i>"
            )
            return
        ...
        cursor = db.cursor()
        cursor.execute(
            "UPDATE crash SET money = money - ? WHERE id = ?", (float(amount), user_id)
        )
        cursor.execute(
            "UPDATE crash SET money = money + ? WHERE id = ?",
            (float(amount), recipient_id),
        )
        db.commit()
        db.close()
        ...
        await message.reply_text(
            "<i>Неправильный формат команды. Используйте: /перевести (количество монет) (ID/@username пользователя)</i>"
        )
    except Exception as e:
        await message.reply_text(f"Произошла ошибка: {e}")
...
async def dice_command(client, message):
    try:
        ...
        if prediction in ["бол", "больше"] and dice_result > 7:
            ...
            prediction.isdigit()
            and 2 <= int(prediction) <= 12
            and dice_result == int(prediction)
        ):
            payout = bet_amount * 5.5
        db = sqlite3.connect(DBB)
        cursor = db.cursor()
        if payout > 0:
            cursor.execute(
                "UPDATE crash SET money = money + ? WHERE id = ?",
                (float(payout - bet_amount), user_id),
            )
            db.commit()
...
async def show_lottery_menu(client, message):
    ...
    await message.reply(
        "🎁 Добро пожаловать в лотерею! Выберите действие:", reply_markup=kb
    )
...
def load_game_db(game_id):
    try:
        with sqlite3.connect(DBB) as db:
            r = db.execute(
                "SELECT game_data FROM mines_games WHERE game_id = ?", (game_id,)
            ).fetchone()
...
async def edit_or_send(client, chat_id, msg_id, text, reply_markup=None):
    ...
    try:
        await client.edit_message_text(
            chat_id, msg_id, text, reply_markup=reply_markup, parse_mode=ParseMode.HTML
        )
        return msg_id
...
async def create_mines_keyboard(
    game_id, opened_cells, mines_positions, exploded, last_index, all_opened
):
    btns = []
    ...
    for i in range(25):
        if i == last_index and exploded:
            ...
            face = "❔"
        btns.append(
            InlineKeyboardButton(face, callback_data=f"mines_cell_{game_id}_{i}")
        )
    ...
    if not exploded and not all_opened:
        if opened_cells:
            markup.inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        "💰 Забрать приз", callback_data=f"mines_take_{game_id}"
                    )
                ]
            )
        else:
            markup.inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        "🚫 Отменить", callback_data=f"mines_cancel_{game_id}"
                    )
                ]
            )
    return markup
...
async def send_game_message(client, game, status="continue", win_amount=None):
    ...
    if status == "lose":
        markup = await create_mines_keyboard(
            gid, opened, mines_positions, True, game.get("last_cell_index"), False
        )
        text = (
        ...
        markup = await create_mines_keyboard(
            gid, opened, mines_positions, False, None, True
        )
        text = (
            f"<b>🎉 Все открыто! Вы выиграли <code>{format_balance(win_amount)}</code></b>\n"
            f"Баланс: <code>{balance}</code>"
        )
    else:  # continue
        markup = await create_mines_keyboard(
            gid, opened, mines_positions, exploded, None, all_opened
        )
        possible = format_balance(int(bet * mult))
        text = (
            f"<b>🟢 Продолжай играть</b>\n"
            f"Открыто: <b>{len(opened)}</b>\n"
            f"Коэффициент: <code>{mult:.2f}x</code>\n"
            f"Возможный выигрыш: <code>{possible}</code>"
        )

    new_id = await edit_or_send(
        client, game["chat_id"], game.get("message_id"), text, reply_markup=markup
    )
...
async def mines_command(client, message):
    ...
    try:
        ...
        with sqlite3.connect(DBB) as db:
            db.execute(
                "UPDATE crash SET money = money - ? WHERE id = ?", (float(bet), uid)
            )
            db.commit()
...
async def mines_cell_callback(client, cq):
    ...
    async with user_locks[uid]:
        try:
            ...
            game["board"] = board
            game["current_multiplier"] = MULTIPLIERS[
                min(num_open, len(MULTIPLIERS) - 1)
            ]
            active_mines_games[gid] = game
            save_game_db(gid, game)
...
async def mines_take_prize_callback(client, cq):
    ...
    async with user_locks[uid]:
        ...
        try:
            with sqlite3.connect(DBB) as db:
                db.execute(
                    "UPDATE crash SET money = money + ? WHERE id = ?", (float(win), uid)
                )
...
async def hhh(client, message):
    ...
    try:
        with sqlite3.connect(DBB) as db:
            db.execute(
                "UPDATE crash SET money = money - ? WHERE id = ?", (money, send_id)
            )
            db.commit()
    except Exception as e:
    ...
    await message.reply(
        f"✅ Вы успешно сняли с баланса пользователя {user.first_name} {format_balance(money)}💰"
    )
    await app.send_message(
        "https://t.me/qpdnsnnfd",
        f"""
<b>Момент: Снятия</b>
...
async def hhh(client, message):
    ...
    user = await client.get_users(send_id)
    await message.reply(
        f"✅ Вы успешно пополнили баланс пользователя {user.first_name} на {format_balance(money)}💰"
    )
    await app.send_message(
        "https://t.me/qpdnsnnfd",
        f"""
...
async def inline_ref_handler(client, inline_query: InlineQuery):
    ...
    if query in ["реф", "ref", "Реф", "Ref"]:
        ...
        if not top_users:
            ...
            text = await format_top_message(
                top_users, "🏆 Топ 10 игроков по балансу:", order_by="money"
            )
...
async def my_refs_handler(client, message):
    ...
    BOT_USERNAME = BOT1.username
    await message.reply(
        f"<b>🔗 Ваша ссылка:</b><code>https://t.me/{BOT_USERNAME}?start=ref_{user_id}</code>\n💰 <b>За одного приглашенного человека вы получите 3.000.000 монет!</b>"
    )
...
async def start_handler(client, message):
        ...
        if not await check_user(user_id):
            ...
            if ref_id and ref_id != user_id and await check_user(ref_id):
                ...
                if cursor.fetchone() is None:
                    cursor.execute(
                        "INSERT INTO ref_system (user_id, invited_by) VALUES (?, ?)",
                        (user_id, ref_id),
                    )
                    cursor.execute(
                        "UPDATE ref_system SET ref_count = ref_count + 1 WHERE user_id = ?",
                        (ref_id,),
                    )
                    cursor.execute(
                        "UPDATE crash SET money = money + ? WHERE id = ?",
                    ...
                    await app.send_message(
                        ref_id,
                        f"Пользователь {message.from_user.first_name} ({user_id}) перешел по вашему рефералу.\nВы получаете 3.000.000 монет за реферальную ссылку.",
                    )
                    await message.reply(
                        "Привет! Я бот, который поможет тебе играть в мины и добывать ресурсы.\n"
                        "Используй /hb или 'список' для просмотра всех команд."
                    ...
                    await message.reply(
                        "Вы уже зарегистрированы. Реферальная ссылка не будет учтена."
                    )
                db.close()
            else:
                await message.reply(
                    "Вы не можете использовать свою ссылку или пользователь не найден."
                )
        else:
    ...
    if len(args) == 2 and args[1].startswith("promo_"):
        ...
        cursor = db.cursor()
        cursor.execute(
            "SELECT money, activations FROM promos WHERE name = ?", (promo_name,)
        )
        promo = cursor.fetchone()
...
async def treasure_game_command(client, message):
    ...
    if len(args) != 3:
        await message.reply_text(
            "Пример: <code>клад 10000 2</code> (ставка, кол-во кладов от 1 до 4)"
        )
        return
    ...
    board = {"treasures": treasure_positions, "opened": []}
    ...
    cursor.execute(
        "INSERT INTO treasure_games (id, user_id, bet, treasures, attempts_left, found, board) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
# ================== КОМАНДЫ ==================
@app.on_message(
    filters.command("farm")
    | filters.regex(r"Фарма", flags=re.IGNORECASE)
    | filters.regex(r"Ферма", flags=re.IGNORECASE)
)
async def farm_menu(client, message):
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "📥 Забрать доход", callback_data=f"claim_{user_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    "💳 Купить устройства", callback_data=f"buuy_menu_{user_id}"
                )
            ],
        ]
    )
        ...
        await callback_query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🔙 Назад", callback_data=f"back_farm_{user_id}"
                        )
                    ]
                ]
            ),
...
async def buy_menu(client, callback_query):
    ...
    for dev_id, dev in farm_devices.items():
        ...
        kb.append(
            [
                InlineKeyboardButton(
                    f"{dev['name']} — {price:,} 💰",
                    callback_data=f"buuy_{dev_id}_{user_id}",
                )
            ]
        )
    ...
    await callback_query.message.edit_text(
        "💳 <b>Выберите устройство для покупки</b>:",
...
async def buy_device(client, callback_query):
    ...
    if user_data is None:
        await callback_query.answer(
            "❌ Не удалось получить данные пользователя.", show_alert=True
        )
        return
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE crash SET money = money - ? WHERE id = ?", (float(price), user_id)
    )
    db.commit()
    db.close()
    ...
    for d_id, dev in farm_devices.items():
        ...
        kb.append(
            [
                InlineKeyboardButton(
                    f"{dev['name']} — {new_price:,} 💰",
                    callback_data=f"buuy_{d_id}_{user_id}",
                )
            ]
        )
    kb.append([InlineKeyboardButton("🔙 Назад", callback_data=f"back_farm_{user_id}")])
...
async def send_treasure_board(client, chat_id, game_id, edit_message=None):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT board, attempts_left, found, treasures, user_id FROM treasure_games WHERE id = ?",
        (game_id,),
    )
    ...
    board, attempts_left, found, treasures, user_id = (
        json.loads(data[0]),
        data[1],
        data[2],
        data[3],
    ...
    for i in range(4):
        ...
        for j in range(4):
            ...
            if pos in opened:
                ...
                label = "⬜"
            row.append(
                InlineKeyboardButton(label, callback_data=f"treasure_{game_id}_{i}_{j}")
            )
        buttons.append(row)

    buttons.append(
        [InlineKeyboardButton("💰 Забрать", callback_data=f"treasure_take_{game_id}")]
    )
...
async def handle_treasure_click(client, callback):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT user_id, bet, treasures, attempts_left, found, board FROM treasure_games WHERE id = ?",
        (game_id,),
    )
    data = cursor.fetchone()
    if not data:
    ...
    async with treasure_locks[game_id]:
        ...
        if found == treasures:
            ...
            cursor.execute(
                "UPDATE crash SET money = money + ? WHERE id = ?", (float(win), user_id)
            )
            cursor.execute("DELETE FROM treasure_games WHERE id = ?", (game_id,))
            db.commit()
            db.close()
            await callback.message.edit_text(
                f"🎉 Все клады найдены! Вы выиграли <b>{format_balance(win)}</b> монет!"
            )
            return
        ...
        if attempts_left <= 0:
            ...
            reveal = "🪙 Клады были тут:\n" + "\n".join(
                [f"({x},{y})" for x, y in board["treasures"]]
            )
            await callback.message.edit_text(
        ...
        cursor.execute(
            "UPDATE treasure_games SET board = ?, attempts_left = ?, found = ? WHERE id = ?",
        ...
        await send_treasure_board(
            client, callback.message.chat.id, game_id, edit_message=callback.message
        )
...
async def handle_treasure_take(client, callback):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT user_id, bet, treasures, found FROM treasure_games WHERE id = ?",
        (game_id,),
    )
    data = cursor.fetchone()
    if not data:
    ...
    async with treasure_locks[game_id]:
        ...
        await callback.message.edit_text(
            f"💰 Забрано! Вы получили <b>{format_balance(win)}</b> монет (за {found}/{treasures} кладов)"
        )
...
async def debug_treasure_game(client, message):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT id, board FROM treasure_games WHERE user_id = ? ORDER BY created_at DESC LIMIT 1",
        (user_id,),
    )
    data = cursor.fetchone()
    db.close()
    ...
    await message.reply_text(
        f"🧭 Игра ID: <code>{game_id}</code>\nКлады:\n"
        + "\n".join([f"({i}, {j})" for i, j in treasures])
    )


def treasure_multiplier(treasures):
    return {1: 3.5, 2: 2.2, 3: 1.7, 4: 1.3}.get(treasures, 1.0)
...
async def send_users_page(client, message, tg_id, users, page):
    ...
    text = (
        f"👥 <b>кол-во игроков:</b> {len(users)}\n"
    ...
    # Добавляем пользователей с нумерацией
    for idx, (user_id, username, first_name, money, status) in enumerate(
        users_page, start=start_idx + 1
    ):
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                "⬅ Прошлая страница", callback_data=f"users_page:{page - 1}"
            )
        )
    if end_idx < len(users):
        nav_buttons.append(
            InlineKeyboardButton(
                "След. страница ➡", callback_data=f"users_page:{page + 1}"
            )
        )
    if nav_buttons:
    ...
    filter_buttons = [
        [
            InlineKeyboardButton(
                f"Все{' ✅' if current_filter == 'all' else ''}",
                callback_data="users_filter:all",
            ),
            InlineKeyboardButton(
                f"Заблокированные{' ✅' if current_filter == 'banned' else ''}",
                callback_data="users_filter:banned",
            ),
        ],
        [
            InlineKeyboardButton(
                f"С деньгами > 1000{' ✅' if current_filter == 'rich' else ''}",
    ...
    sort_buttons = [
        [
            InlineKeyboardButton(
                f"Деньги (много){' ✅' if current_sort == 'money_high' else ''}",
                callback_data="users_sort:money_high",
            ),
            InlineKeyboardButton(
                f"Деньги (мало){' ✅' if current_sort == 'money_low' else ''}",
                callback_data="users_sort:money_low",
            ),
        ],
        [
            InlineKeyboardButton(
                f"Активные{' ✅' if current_sort == 'inactive' else ''}",
                callback_data="users_sort:inactive",
            ),
            InlineKeyboardButton(
                f"Неактивные{' ✅' if current_sort == 'active' else ''}",
                callback_data="users_sort:active",
            ),
        ],
        [
            InlineKeyboardButton(
                f"По умолчанию{' ✅' if current_sort == 'default' else ''}",
...
async def change_users_filter(client, callback_query):
    ...
    if not users:
        await callback_query.answer(
            "Пользователей по этому фильтру нет.", show_alert=True
        )
        return
...
async def buy_drill_command(client, message):
    ...
    await message.reply(
        f"🎉 Вы купили дрель за {cost} монет! Теперь можно добывать ресурсы с помощью /mine."
    )
...
async def duel_command(client, message):
    ...
    for game in duel_data.values():
        if game["player1_id"] == user_id or game.get("player2_id") == user_id:
            ...
            btn = InlineKeyboardButton(
                "❌ Отменить дуэль", callback_data=f"duel_cancel_{game['game_id']}"
            )
            await message.reply(
                "У вас уже есть активная дуэль!",
    ...
    markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(str(i), callback_data=f"duel_select_{game_id}_{i}")
                for i in range(1, 4)
            ],
            [
                InlineKeyboardButton(str(i), callback_data=f"duel_select_{game_id}_{i}")
    ...
    msg = await message.reply(
        "Вы создаёте игру. Выберите число от 1 до 6:", reply_markup=markup
    )
    ...
    async with duel_locks[game_id]:
        ...
        markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "✅ Принять вызов", callback_data=f"duel_accept_{game_id}"
                    )
                ],
                [
                    InlineKeyboardButton(
                        "❌ Отмена", callback_data=f"duel_cancel_{game_id}"
                    )
                ],
            ]
        )
    ...
    async with duel_locks[game_id]:
        ...
        try:
            with sqlite3.connect(DBB) as db:
                db.execute(
                    "UPDATE crash SET money = money + ? WHERE id = ?",
                    (float(game["bet"]), game["player1_id"]),
                )
                db.commit()
        except sqlite3.Error:
...
async def duel_accept_callback(client, cq):
    ...
    if game["state"] != "awaiting_opponent":
        ...
        return
    if any(
        g["player1_id"] == user_id or g.get("player2_id") == user_id
        for g in duel_data.values()
    ):
    ...
    async with duel_locks[game_id]:
        ...
        markup = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        str(i), callback_data=f"duel_opponent_{game_id}_{i}"
                    )
                    for i in range(1, 4)
                ],
                [
    ...
    if not available:
        try:
            with sqlite3.connect(DBB) as db:
                ...
                db.execute(
                    "UPDATE crash SET money = money + ? WHERE id = ?",
                    (float(game["bet"]), game["player2_id"]),
                )
                db.commit()
        except sqlite3.Error:
            pass
    ...
    if dice_result == game["player1_num"] and dice_result == game["player2_num"]:
        try:
            with sqlite3.connect(DBB) as db:
                db.execute(
                    "UPDATE crash SET money = money + ? WHERE id = ?",
                    (float(current_bet // 2), game["player1_id"]),
                )
                db.execute(
                    "UPDATE crash SET money = money + ? WHERE id = ?",
                    (float(current_bet // 2), game["player2_id"]),
                )
                db.commit()
        except sqlite3.Error:
            pass
...
async def global_get_money(client, message):
    ...
    if len(message.text.split()) < 2:
        await message.reply(
            "❌ Укажите сколько монет выдать игрокам: /ggm [количество]"
        )
        return
...
def build_tower_keyboard(game_id: str, state: dict) -> InlineKeyboardMarkup:
    ...
    buttons = [
        InlineKeyboardButton("❔", callback_data=f"tower_choose_{game_id}_{i}")
        for i in range(5)
    ]
    kb.append(buttons)
    ...
    if level == 0:
        kb.append(
            [InlineKeyboardButton("Отмена", callback_data=f"tower_cancel_{game_id}")]
        )
    else:
        kb.append(
            [
                InlineKeyboardButton(
                    "Забрать приз", callback_data=f"tower_collect_{game_id}"
                )
            ]
        )
...
async def start_tower(client: Client, message: Message):
    ...
    if len(parts) != 2:
        return await message.reply_text(
            "Используйте: Башня <ставка>, например: Башня 300 или Башня 1к"
        )
    arg = parts[1]
    ...
    if len(active_list) >= 2:
        return await message.reply_text(
            "У вас уже 2 активные игры. Закончите одну, прежде чем начинать новую."
        )
...
async def handle_collect(client: Client, callback: CallbackQuery):
    ...
    async with lock:
        if state.get("finished"):
            return await callback.answer("Уже забрано.", show_alert=True)
        state["finished"] = True
...
async def handle_cancel(client: Client, callback: CallbackQuery):
    ...
    async with lock:
        ...
        try:
            await callback.message.edit_text(
                f"Игра отменена, {format_balance(bet)} возвращены."
            )
        except Exception:
            pass
        remove_tower_game(game_id)
...
async def test_bank_interest(client, message):
    ...
    with sqlite3.connect(DBB) as db:
        cursor = db.cursor()
        cursor.execute(
            "SELECT deposit_id, current_amount FROM bank_deposits WHERE is_active = 1"
        )
        deposits = cursor.fetchall()
        for deposit_id, current_amount in deposits:
            new_amount = int(current_amount * 1.05)  # +5%
            cursor.execute(
                "UPDATE bank_deposits SET current_amount = ? WHERE deposit_id = ?",
                (float(new_amount), deposit_id),
            )
        db.commit()

    await message.reply(
        f"[BANK] Начислены проценты для {len(deposits)} депозитов в {now.strftime('%Y-%m-%d %H:%M:%S %Z')}"
    )
    print(
        f"[BANK] Начислены проценты для {len(deposits)} депозитов в {now.strftime('%Y-%m-%d %H:%M:%S %Z')}"
    )
...
async def bank_command(client, message):
    ...
    if len(parts) == 1:
        ...
        with sqlite3.connect(DBB) as db:
            cursor = db.cursor()
            cursor.execute(
                "SELECT deposit_id, amount, current_amount, created_at FROM bank_deposits WHERE user_id = ? AND is_active = 1",
                (user_id,),
            )
            deposits = cursor.fetchall()
        ...
        for deposit in deposits:
            ...
            response += (
                f"📌 <b>Депозит ID:</b> <code>{deposit_id[:8]}</code>\n"
        ...
        inline_keyboard = [
            [
                InlineKeyboardButton(
                    f"Закрыть депозит {deposit[0][:8]}",
                    callback_data=f"bank_close_{deposit[0]}",
                )
            ]
            for deposit in deposits
        ]
        ...
        response += (
            f"<b>💰 Баланс:</b> <code>{format_balance(user_data['money'])}</code>\n"
        )
    ...
    if len(parts) == 2:
        ...
        if input_amount in ("все", "всё"):
            ...
            if amount is None:
                await message.reply(
                    "<i>Укажите сумму депозита числом (например, 1000, 100к, 1.5кк) или 'все'</i>"
                )
                return
        ...
        if amount == 0:
            await message.reply(
                "<i>Ваш баланс равен 0. Пополните баланс, чтобы создать депозит.</i>"
            )
            return
        ...
        with sqlite3.connect(DBB) as db:
            cursor = db.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM bank_deposits WHERE user_id = ? AND is_active = 1",
                (user_id,),
            )
        ...
        if deposit_count >= 10:
            await message.reply(
                "<i>У вас уже 10 активных депозитов. Закройте один, чтобы создать новый.</i>"
            )
            return
        ...
        with sqlite3.connect(DBB) as db:
            ...
            cursor.execute(
                "INSERT INTO bank_deposits (deposit_id, user_id, amount, current_amount) VALUES (?, ?, ?, ?)",
...
async def bank_close_callback(client, callback_query):
    ...
    async with user_locks[user_id]:
        with sqlite3.connect(DBB) as db:
            cursor = db.cursor()
            cursor.execute(
                "SELECT current_amount FROM bank_deposits WHERE deposit_id = ? AND user_id = ? AND is_active = 1",
                (deposit_id, user_id),
            )
            ...
            cursor.execute(
                "UPDATE crash SET money = money + ? WHERE id = ?",
                (float(current_amount), user_id),
            )
            cursor.execute(
                "UPDATE bank_deposits SET is_active = 0 WHERE deposit_id = ?",
                (deposit_id,),
            )
            db.commit()
...
async def add_shop_item(client, message):
    ...
    if len(parts) < 4:
        await message.reply(
            "Используйте: /fff (название) (цена) (количество)\nПример: /fff меч 1000 5"
        )
        return
    ...
    try:
        cursor.execute(
            "INSERT INTO shop (name, price, quantity) VALUES (?, ?, ?)",
            (name, price, quantity),
        )
        db.commit()
        await message.reply(
            f"Предмет '{name}' добавлен в магазин!\nЦена: {price} монет\nКоличество: {quantity}"
        )
    except sqlite3.IntegrityError:
...
async def update_shop_message(client, message, user_id, items, page):
    ...
    text = (
        f"🛒 <b>Магазин (страница {page + 1}/{((len(items) - 1) // SHOP_PAGE_SIZE) + 1}):</b>\n"
    ...
    for name, price, quantity in items_page:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    button_text, callback_data=f"shop_info:{name}:{page}"
                )
            ]
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅ Назад", callback_data=f"shop_page:{page - 1}")
        )
    if end_idx < len(items):
        nav_buttons.append(
            InlineKeyboardButton("Вперед ➡", callback_data=f"shop_page:{page + 1}")
        )
    if nav_buttons:
        buttons.append(nav_buttons)
    ...
    sort_buttons = [
        [
            InlineKeyboardButton(
                f"Дешевые{' ✅' if current_sort == 'cheap' else ''}",
                callback_data="shop_sort:cheap",
            ),
            InlineKeyboardButton(
                f"Дорогие{' ✅' if current_sort == 'expensive' else ''}",
                callback_data="shop_sort:expensive",
            ),
        ],
        [
            InlineKeyboardButton(
                f"Новые{' ✅' if current_sort == 'new' else ''}",
                callback_data="shop_sort:new",
            ),
            InlineKeyboardButton(
                f"Старые{' ✅' if current_sort == 'old' else ''}",
                callback_data="shop_sort:old",
            ),
        ],
        [
            InlineKeyboardButton(
                f"Кол-во (много){' ✅' if current_sort == 'quantity_high' else ''}",
                callback_data="shop_sort:quantity_high",
            ),
            InlineKeyboardButton(
                f"Кол-во (мало){' ✅' if current_sort == 'quantity_low' else ''}",
                callback_data="shop_sort:quantity_low",
            ),
        ],
        [
            InlineKeyboardButton(
                f"По умолчанию{' ✅' if current_sort == 'default' else ''}",
                callback_data="shop_sort:default",
            )
        ],
    ]
    buttons.extend(sort_buttons)
...
async def shop_item_info(client, callback_query):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT name, price, quantity FROM shop WHERE name = ?", (item_name,)
    )
    ...
    display_qty = (
        "❌" if is_out_of_stock else f"Всего: {quantity if quantity > 0 else '∞'}"
    )
    user_data = get_user_data(user_id)
...
async def shop_buy_item(client, callback_query):
    ...
    with sqlite3.connect(DBB) as db:
        ...
        if quantity != -1:
            cursor.execute(
                "UPDATE shop SET quantity = quantity - 1 WHERE name = ?", (item_name,)
            )
        cursor.execute(
            """
    ...
    display_qty = (
        "Товар нет в наличии"
        if is_out_of_stock
        else f"Осталось в магазине: {quantity if quantity > 0 else '∞'}"
    )
    new_balance = user_data["money"] - price

    text = (
        f"🎉 <b>Вы купили: {name}</b>\n"
        f"💰 Цена: {format_balance(price)} \n"
        f"📦 {display_qty}\n"
    ...
    if is_out_of_stock:
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "Купить еще", callback_data=f"shop_buy:{name}:{page}"
                )
            ],
            [InlineKeyboardButton("Назад", callback_data=f"shop_back:{page}")],
        ]
...
async def sell_item(client, message):
    ...
    if len(args) < 4:
        return await message.reply(
            "Использование: /sell [название_предмета] [цена] [кол-во]"
        )
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT quantity FROM inv_user WHERE user_id = ? AND item_name = ?",
        (user_id, item),
    )
    result = cursor.fetchone()
    ...
    await message.reply(
        f"✅ Вы выставили <b>{item}</b> ×{amount} по <b>{format_balance(price)}</b> монет."
    )
...
async def market_buy_main(client, callback_query):
    ...
    buttons = [
        [InlineKeyboardButton(f"{item[0]}", callback_data=f"buy_item_group_{item[0]}")]
        for item in items
    ]
    markup = InlineKeyboardMarkup(buttons)
    await callback_query.message.edit_text(
        "🔎 Выберите предмет для покупки:", reply_markup=markup
    )
    ...
    try:
        ...
        if user:
            return (
                ...
                else user.first_name or str(seller_id)
            )
    except Exception as e:
...
async def market_item_group(client, callback_query):
    ...
    try:
        ...
        if not offers:
            try:
                await callback_query.message.edit_text(
                    "❌ Этих предметов больше нет в продаже."
                )
...
async def buy_confirm(client, callback_query):
    ...
    try:
        ...
        buttons = [
            [
                InlineKeyboardButton(
                    "Подтвердить покупку", callback_data=f"buy_final_{market_id}_{item}"
                )
            ],
        ...
        logger.error(
            f"Ошибка в buy_confirm (market_id={locals().get('market_id')}, item={locals().get('item')}): {e}"
        )
        await callback_query.answer("Произошла ошибка.", show_alert=True)
    ...
    try:
        ...
        with closing(sqlite3.connect(DBB)) as db:
            ...
            if user_id == seller_id:
                try:
                    await callback_query.message.edit_text(
                        "❌ Нельзя купить предмет у самого себя."
                    )
                except MessageNotModified:
                    pass
                return
            ...
            new_message_text = (
                f"🎉 Вы купили <b>{item}</b> за <b>{format_balance(price)}</b> монет."
            )

            # Проверяем, нужно ли редактировать сообщение
            if new_message_text == current_message_text:
                logger.debug(
                    f"Сообщение не изменено для market_id={market_id}, пропускаем редактирование."
                )
            ...
            try:
                # Обновляем деньги
                cursor.execute(
                    "UPDATE crash SET money = money - ? WHERE id = ?", (price, user_id)
                )
                ...
                if quantity <= 1:
                    cursor.execute(
                        "DELETE FROM MARKETPLACE WHERE rowid = ?", (market_id,)
                    )
                else:
                    cursor.execute(
                    ...
                    if cursor.rowcount == 0:
                        raise Exception(
                            "Не удалось обновить количество на маркетплейсе."
                        )
                ...
                db.rollback()
                logger.error(
                    f"Ошибка при покупке предмета (market_id={market_id}): {e}"
                )
                try:
                    await callback_query.message.edit_text(
        ...
        if quantity > 1:
            buttons.append(
                [
                    InlineKeyboardButton(
                        "Купить еще", callback_data=f"buy_item_group_{item}"
                    )
                ]
            )
        ...
        try:
            await callback_query.message.edit_text(
                new_message_text, reply_markup=InlineKeyboardMarkup(buttons)
            )
        except MessageNotModified:
        ...
        try:
            seller_info = get_user_data(seller_id) or {
                "username": str(seller_id),
                "id": seller_id,
            }
            await app.send_message(
                "-1003122575028",
...
<b>Дата:</b> {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        ...
        try:
            ...
            try:
                await app.send_message(
                    seller_id_int,
                    f"Ваш предмет {item} продан игроку {callback_query.from_user.first_name} за {format_balance(price)} монет.",
                )
        ...
        logger.error(
            f"Ошибка в buy_final (market_id={locals().get('market_id')}, item={locals().get('item')}): {e}"
        )
        try:
...
async def validate_item_quantity(
    user_id: int, item: str, quantity: int
) -> Tuple[bool, str]:
...
def create_quantity_keyboard(
    item: str, max_quantity: int, current_quantity: int
    ...
    buttons = [
        [
            InlineKeyboardButton(
                "+1",
                callback_data=f"sell_qty:{item}:{min(max_quantity, current_quantity + 1)}",
            ),
            InlineKeyboardButton(
                "+10",
                callback_data=f"sell_qty:{item}:{min(max_quantity, current_quantity + 10)}",
            ),
            InlineKeyboardButton(
                "+100",
                callback_data=f"sell_qty:{item}:{min(max_quantity, current_quantity + 100)}",
            ),
            InlineKeyboardButton(
                "Все", callback_data=f"sell_qty:{item}:{max_quantity}"
            ),
        ],
        [
            InlineKeyboardButton(
                "-1", callback_data=f"sell_qty:{item}:{max(0, current_quantity - 1)}"
            ),
            InlineKeyboardButton(
                "-10", callback_data=f"sell_qty:{item}:{max(0, current_quantity - 10)}"
            ),
            InlineKeyboardButton(
                "-100",
                callback_data=f"sell_qty:{item}:{max(0, current_quantity - 100)}",
            ),
    ...
    if current_quantity > 0:
        buttons.append(
            [
                InlineKeyboardButton(
                    "Подтвердить", callback_data=f"sell_price:{item}:{current_quantity}"
                )
            ]
        )
...
def create_price_keyboard(
    item: str, quantity: int, current_price: int
) -> InlineKeyboardMarkup:
    ...
    buttons = [
        [
            InlineKeyboardButton(
                "+100",
                callback_data=f"sell_price_adj:{item}:{quantity}:{current_price + 100}",
            ),
            InlineKeyboardButton(
                "+1к",
                callback_data=f"sell_price_adj:{item}:{quantity}:{current_price + 1000}",
            ),
            InlineKeyboardButton(
                "+10к",
                callback_data=f"sell_price_adj:{item}:{quantity}:{current_price + 10000}",
            ),
            InlineKeyboardButton(
                "+100к",
                callback_data=f"sell_price_adj:{item}:{quantity}:{current_price + 100000}",
            ),
            InlineKeyboardButton(
                "+1кк",
                callback_data=f"sell_price_adj:{item}:{quantity}:{current_price + 1000000}",
            ),
        ],
        [
            InlineKeyboardButton(
                "-100",
                callback_data=f"sell_price_adj:{item}:{quantity}:{max(0, current_price - 100)}",
            ),
            InlineKeyboardButton(
                "-1к",
                callback_data=f"sell_price_adj:{item}:{quantity}:{max(0, current_price - 1000)}",
            ),
            InlineKeyboardButton(
                "-10к",
                callback_data=f"sell_price_adj:{item}:{quantity}:{max(0, current_price - 10000)}",
            ),
            InlineKeyboardButton(
                "-100к",
                callback_data=f"sell_price_adj:{item}:{quantity}:{max(0, current_price - 100000)}",
            ),
            InlineKeyboardButton(
                "-1кк",
                callback_data=f"sell_price_adj:{item}:{quantity}:{max(0, current_price - 1000000)}",
            ),
        ],
    ]
    buttons.append(
        [
            InlineKeyboardButton(
                "Сброс", callback_data=f"sell_price_adj:{item}:{quantity}:0"
            )
        ]
    )
    if current_price > 0:
        buttons.append(
            [
                InlineKeyboardButton(
                    "Подтвердить цену",
                    callback_data=f"sell_confirm:{item}:{quantity}:{current_price}",
                )
            ]
        )
    buttons.append(
        [InlineKeyboardButton("Назад", callback_data=f"sell_qty:{item}:{quantity}")]
    )
    return InlineKeyboardMarkup(buttons)
...
async def market_sell_choose_item(client, callback_query):
    ...
    try:
        with closing(sqlite3.connect(DBB)) as db:
            cursor = db.cursor()
            cursor.execute(
                "SELECT item_name, quantity FROM inv_user WHERE user_id = ? AND quantity > 0",
                (user_id,),
            )
            items = cursor.fetchall()
...
async def sell_item_selection(client, callback_query):
    ...
    try:
        ...
        if not result or result[0] <= 0:
            await callback_query.message.edit_text(
                "❌ У вас нет этого предмета для продажи."
            )
            return
...
async def confirm_sell(client, callback_query):
    ...
    try:
        ...
        if quantity <= 0:
            await callback_query.answer(
                "❌ Выберите количество больше 0.", show_alert=True
            )
            return
        if price <= 0:
            await callback_query.answer(
                "❌ Цена должна быть больше 0.", show_alert=True
            )
            return
        ...
        with closing(sqlite3.connect(DBB)) as db:
            ...
            try:
                ...
                if cursor.rowcount == 0:
                    raise Exception(
                        "Не удалось обновить инвентарь: предмет не найден или количество не обновлено."
                    )
                ...
                await callback_query.answer(
                    "❌ Ошибка при добавлении предмета на маркетплейс.", show_alert=True
                )
                return
...
async def market_my_lots(client, callback_query):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT rowid, item_name, price, quantity FROM MARKETPLACE WHERE seller_id = ?",
        (user_id,),
    )
    offers = cursor.fetchall()
    db.close()
    ...
    if not offers:
        return await callback_query.message.edit_text(
            "📋 У вас нет выставленных предметов."
        )
    ...
    for rowid, item, price, quantity in offers:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    f"❌ Убрать {item}", callback_data=f"remove_offer_{rowid}"
                )
            ]
        )
...
async def remove_offer(client, callback_query):
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT item_name, quantity FROM MARKETPLACE WHERE rowid = ? AND seller_id = ?",
        (rowid, user_id),
    )
    offer = cursor.fetchone()
    if not offer:
...
async def buy_item(client, callback_query):
    ...
    try:
        ...
        with closing(sqlite3.connect(DBB)) as db:
            ...
            if not offer:
                await callback_query.answer(
                    "❌ Предмет уже куплен или удален.", show_alert=True
                )
                return
            ...
            if user_id == seller_id:
                await callback_query.answer(
                    "❌ Нельзя купить предмет у самого себя.", show_alert=True
                )
                return
            ...
            try:
                ...
                await callback_query.answer(
                    "❌ Ошибка при покупке предмета.", show_alert=True
                )
                return
        ...
        try:
            ...
            logger.debug(
                f"Сообщение не изменено для market_id={market_id}, игнорируем ошибку."
            )
            await callback_query.answer("Покупка уже обработана.", show_alert=True)
            return
...
async def update_inventory_message(client, message, user_id, items, page):
    ...
    text = (
        f"🎒 <b>Ваш инвентарь (страница {page + 1}/{((len(items) - 1) // INV_PAGE_SIZE) + 1}):</b>\n"
        f"Всего предметов: {len(items)}\n"
        f"Сортировка: {sort_display[current_sort]}\n\n"
        f"Список предметов:"
    )
    ...
    for item_name, quantity, price, created_at in items_page:
        if quantity == 0:
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        button_text, callback_data=f"inv_info:{item_name}:{page}"
                    )
                ]
            )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅ Назад", callback_data=f"inv_page:{page - 1}")
        )
    if end_idx < len(items):
        nav_buttons.append(
            InlineKeyboardButton("Вперед ➡", callback_data=f"inv_page:{page + 1}")
        )
    if nav_buttons:
        buttons.append(nav_buttons)
    ...
    sort_buttons = [
        [
            InlineKeyboardButton(
                f"Дешевые{' ✅' if current_sort == 'cheap' else ''}",
                callback_data="inv_sort:cheap",
            ),
            InlineKeyboardButton(
                f"Дорогие{' ✅' if current_sort == 'expensive' else ''}",
                callback_data="inv_sort:expensive",
            ),
        ],
        [
            InlineKeyboardButton(
                f"Новые{' ✅' if current_sort == 'new' else ''}",
                callback_data="inv_sort:new",
            ),
            InlineKeyboardButton(
                f"Старые{' ✅' if current_sort == 'old' else ''}",
                callback_data="inv_sort:old",
            ),
        ],
        [
            InlineKeyboardButton(
                f"Кол-во (много){' ✅' if current_sort == 'quantity_high' else ''}",
                callback_data="inv_sort:quantity_high",
            ),
            InlineKeyboardButton(
                f"Кол-во (мало){' ✅' if current_sort == 'quantity_low' else ''}",
                callback_data="inv_sort:quantity_low",
            ),
        ],
        [
            InlineKeyboardButton(
                f"По умолчанию{' ✅' if current_sort == 'default' else ''}",
                callback_data="inv_sort:default",
            )
        ],
    ]
    buttons.extend(sort_buttons)
...
async def handle_voice_command(client, message):
    ...
    try:
        subprocess.run([FFMPEG_PATH, "-i", ogg_path, wav_path], check=True)
...
async def join_channel_command(client: Client, message):
    ...
    try:
        ...
        if "USER_ALREADY_PARTICIPANT" in str(e):
            ...
            await message.reply_text(
                "Бот не может подписаться, так как требуется приглашение от администратора канала."
            )
        else:
...
async def start_safe(client, message: Message):
    ...
    if len(parts) < 2 or not parts[1].strip():
        return await message.reply_text(
            "⚠ Использование: /safe <ставка> или сейф <ставка>"
        )
    ...
    cancel_btn = [
        InlineKeyboardButton("❌ Отменить игру", callback_data=f"safe_cancel:{user_id}")
    ]
...
async def send_data_db(client, message):
    ...
    try:
        await message.reply_document(document=file_path, caption="Вот твоя бд")
    except Exception as e:
...
async def handle_text_commands(client: Client, message):
    ...
    if await antispam(client, message):
        await app.send_message(
            message.chat.id, f" {message.from_user.first_name} не спамь!"
        )
        return
    ...
    try:
        ...
        if member.status not in [
            ...
            await message.reply_text(
                "Чтобы играть, подпишитесь на канал 👇", reply_markup=btn
            )
            return
    except Exception as e:
...
config = {
    ...
    "r_adjustment": {2: 0.05, 3: 0.1, 4: 0.15, 5: 0.2},
}
...
async def crash_command(client, message):
    ...
    try:
        ...
        if len(parts) != 3:
            await message.reply_text(
                "<i>Используйте: краш (сумма ставки) (икс)</i>\nПример: краш 100к 2.5"
            )
            return
        ...
        if not re.match(r"^\d+(\.\d{1,2})?$", multiplier_str):
            await message.reply_text(
                "<i>Неправильный формат множителя (например, 2.5).</i>"
            )
            return
        ...
        with sqlite3.connect(DBB) as db:
            ...
            if crash_point >= multiplier:
                ...
                cursor.execute(
                    "UPDATE crash SET money = money + ? WHERE id = ?",
                    (float(win_amount - bet_amount), user_id),
                )
                db.commit()
